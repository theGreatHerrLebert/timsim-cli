...
| `timsim-frag-ce` | `tdf` | Per-precursor **collision energy**, in the two flavours a timsTOF actually has. **dda-PASEF** (default): CCS → `1/K0` → mobility scan → CE, using the run's own Bruker calibration and the same `ActivationPolicy` v1's dda-PASEF selection drives. **dia-PASEF** (`--dia`): there is no ramp — the CE is read out of the reference `.d`'s own `DiaFrameMsMsWindows` table, per the window that isolates the precursor, through v1's own `TimsTofCollisionEnergyDIA`. On the benchmark reference that table spans **20.00–58.12 eV**, against which a flat 25.0 eV is off by −33.1/+5.0 eV. An *optional* input to `timsim-fragments` (`--collision-energies`) — without it that stage keeps predicting every precursor at one CE, which is right for Astral and wrong for the timsTOF. |
