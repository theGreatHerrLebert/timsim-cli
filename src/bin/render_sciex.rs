...
fn main() -> Result<()> {
    ...
    // `--n-sigma` was NOT validated here at all (only `--sigma-seconds` was), so a NaN radius
    // reached `elution_half_widths` and produced an empty active set. One shared validator now.
